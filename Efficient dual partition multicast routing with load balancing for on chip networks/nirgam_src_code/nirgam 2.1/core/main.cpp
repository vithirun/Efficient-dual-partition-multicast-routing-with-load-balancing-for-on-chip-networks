...
//cout<<"hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii";
    ...
    {
        cout<<"case1";
        cout<<"nirgam.does not exist, using default parameter values."<<endl;
    }
    else
    {
        cout<<"Case 2";
...
//////////
	...
	float noc_total_throughput=0.0;
   // extern ULL t_p;
    for (int i = 0; i < num_rows; i++)
    {
        for (int j = 0; j < num_cols; j++)
		...
		//cout<<"avfgfjgd : "<<return_avgthroughput()<<endl;   
   }
    }
//	noc_total_throughput = (noc.nwtile[0][0])->return_avgthroughput();  
	t_p=noc_total_packets;
	cout<<"totpackets"<<t_p<<endl;
...
//	results_log<<"\n Average throughput = "<<noc_total_throughput<<endl;
    if (qos==1)
    {
   ...
   // results_log<<"Total Flits Received = "<<total_flits_recvd<<endl;
    ...
    power_results_log<<endl<<endl<<"\nTotal Network Power(In Watt):"<<totalPower;	




   // power_results_log<<endl<<endl<<"\nTotal Link Power(In Watt):"<<totalLinkPower;
